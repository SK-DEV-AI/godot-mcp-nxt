...
export class AsyncOperationQueue {
    ...
    async add(operation, options = {}) {
        const { priority = 0, timeout = this.defaultTimeout, id = this.generateId() } = options;
...
export class GodotOperationQueue extends AsyncOperationQueue {
    constructor(maxConcurrency = 2) {
        ...
        this.resourceLocks = new Map(); // resource -> Set of operation IDs
    }
    async addWithResourceLock(operation, resources, options = {}) {
    ...
    getConflictingOperations(resources) {
        const conflicts = [];
        for (const resource of resources) {
            const locks = this.resourceLocks.get(resource);
...
let globalGodotQueue = null;
let globalGeneralQueue = null;
export function getGodotOperationQueue() {
    if (!globalGodotQueue) {
        globalGodotQueue = new GodotOperationQueue();
    }
    return globalGodotQueue;
}
export function getGeneralOperationQueue() {
