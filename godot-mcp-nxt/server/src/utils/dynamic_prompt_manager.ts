...
export class DynamicPromptManager {
  ...
  private injectionHistory: PromptInjection[] = [];
  ...
  getInjectionStatistics(): InjectionStatistics {
    ...
    const templateStats: { [key: string]: TemplateStats } = {};
  ...
  private async enrichPrompt(basePrompt: string, context: PromptContext): Promise<string> {
  ...
  private async applyInjections(enrichedPrompt: string, context: PromptContext): Promise<string> {
  ...
  private async addGodotContext(prompt: string, godotContext: GodotContext): Promise<string> {
  ...
  private addUserContext(prompt: string, userContext: UserContext): string {
    let contextString = "\n\n## User Context\n";
  ...
  private addSessionContext(prompt: string, sessionContext: SessionContext): string {
    let contextString = "\n\n## Session Context\n";
  ...
  private async analyzeContext(context: PromptContext): Promise<ContextAnalysis> {
  ...
  private async getProjectSpecificSuggestions(context: PromptContext): Promise<PromptSuggestion[]> {
    const suggestions: PromptSuggestion[] = [];
  ...
  private async applyTemplate(prompt: string, template: PromptTemplate, context: PromptContext): Promise<string> {
  ...
  private async extractTemplateVariables(context: PromptContext): Promise<{ [key: string]: any }> {
  ...
  private async generateDynamicInjections(context: PromptContext): Promise<DynamicInjection[]> {
  ...
  private injectContent(prompt: string, injection: DynamicInjection): string {
...
- Optimize for complex scene management and NPC AI`,
  ...
  private getTemplate(name: string): PromptTemplate | undefined {
    return this.promptTemplates.get(name);
  }

  private storeInjection(injection: PromptInjection): void {
...
// Singleton instance
let promptManagerInstance: DynamicPromptManager | null = null;

export function getPromptManager(): DynamicPromptManager {
...
export interface InjectionStatistics {
  ...
  templateStats: { [key: string]: TemplateStats };
