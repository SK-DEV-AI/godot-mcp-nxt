...
export class EnhancedErrorHandler {
  ...
  async applyRecovery(errorId: string, strategyIndex: number): Promise<RecoveryResult> {
    ...
    try {
      ...
      return {
        ...
        message: `Failed to apply recovery strategy: ${(executionError as Error).message}`
      };
    }
  }
  ...
  learnFromResolution(errorRecord: ErrorRecord): void {
    if (!errorRecord.resolved || !errorRecord.resolution) return;
  ...
  private determineSeverity(error: ErrorContext): ErrorSeverity {
  ...
  private categorizeError(error: ErrorContext): string {
    const message = error.message.toLowerCase();
  ...
  private findSimilarErrors(error: ErrorContext): ErrorRecord[] {
    return this.errorHistory
      .filter(record => this.isSimilarError(record.error, error))
      .slice(-5); // Return last 5 similar errors
  }

  private isSimilarError(error1: ErrorContext, error2: ErrorContext): boolean {
    ...
    const commonWords = messageWords1.filter(word =>
      messageWords2.includes(word) && word.length > 3
    );
  ...
  private calculateStrategyConfidence(strategy: RecoveryStrategy, error: ErrorContext): number {
  ...
  private calculateConfidence(error: ErrorContext, analysis: ErrorAnalysis): number {
  ...
  private async executeRecoveryStrategy(
    ...
    try {
      ...
      return {
        success: false,
        error: executionError as Error,
        message: `Recovery execution failed: ${(executionError as Error).message}`
      };
    }
  }

  private storeErrorRecord(record: ErrorRecord): void {
