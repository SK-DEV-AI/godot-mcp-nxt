 ...
 */
export const advancedEditorTools: MCPTool[] = [
  {
    ...
    execute: async ({ script_code, timeout, capture_output, include_stack_trace }): Promise<string> => {
      ...
      try {
        ...
        const errorMessage = `Failed to execute editor script: ${(error as Error).message}`;
    ...
    execute: async ({ clear_errors, clear_warnings, clear_info, preserve_recent }): Promise<string> => {
      ...
      try {
        ...
        if (result.cleared_counts) {
          ...
          if (counts.errors > 0) response += `- Errors: ${counts.errors}\n`;
          if (counts.warnings > 0) response += `- Warnings: ${counts.warnings}\n`;
          if (counts.info > 0) response += `- Info: ${counts.info}\n`;
          response += `- Total: ${counts.total}\n\n`;
        }

        if (preserve_recent && result.preserved_lines) {
          response += `📄 **Preserved Recent Messages:**\n`;
          result.preserved_lines.forEach((line: string, index: number) => {
...

  {
    ...
    execute: async ({ lines, filter_type, include_timestamps, search_pattern }): Promise<string> => {
      ...
      try {
        ...
        return `❌ Failed to retrieve editor logs: ${(error as Error).message}\n\n🔧 **Troubleshooting:**\n- Ensure Godot Editor is running\n- Check console accessibility\n- Try with fewer lines or no filter`;
      }
    },
  },

  {
    ...
    execute: async ({
      ...
      try {
        ...
        const errorMessage = `Failed to export scene screenshot: ${(error as Error).message}`;
