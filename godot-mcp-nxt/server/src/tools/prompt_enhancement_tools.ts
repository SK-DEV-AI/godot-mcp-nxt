 ...
 */
export const promptEnhancementTools: MCPTool[] = [
  {
    name: 'enhance_prompt',
...
- Project-type specific guidance (2D platformer, 3D RPG, etc.)`,
    ...
    execute: async (params: any): Promise<string> => {
      ...
      try {
        ...
        throw new Error(`Prompt enhancement operation failed: ${(error as Error).message}`);
      }
    },
  },
...
- Code structure and maintainability`,
    ...
    execute: async (params: any): Promise<string> => {
      ...
      try {
        ...
        throw new Error(`Context-aware assistance failed: ${(error as Error).message}`);
      }
    },
  }
];
 ...
 */
async function buildPromptContext(params: any): Promise<any> {
  const context: any = {
...
async function generateContextualGuidance(contextType: string, context: any, detailLevel: string): Promise<any> {
  const guidance: any = {
    type: contextType,
    recommendations: [],
    examples: [],
...
async function analyzeProjectState(focusArea?: string): Promise<any> {
  const analysis: any = {
...
async function getBestPractices(category: string): Promise<any> {
  const practices: any = {
    category: category,
    practices: [],
    examples: []
...
async function generatePerformanceGuidance(currentFps: number): Promise<any> {
  const guidance: any = {
...
async function generateWorkflowAssistance(workflowStep?: string): Promise<any> {
 ...
 */
function formatEnhancedPrompt(original: string, enhanced: string, context: any): string {
...
function formatContextAnalysis(context: any, suggestions: any[]): string {
...
function formatImprovementSuggestions(suggestions: any[], currentPrompt: string): string {
...
function formatInjectionStatistics(stats: any): string {
  ...
  if (stats.templateStats && Object.keys(stats.templateStats).length > 0) {
    ...
    Object.entries(stats.templateStats).forEach(([name, template]: [string, any]) => {
...
function formatGuidanceResponse(guidance: any, includeExamples: boolean): string {
  ...
  if (includeExamples && guidance.examples && guidance.examples.length > 0) {
    output += "\n**Examples:**\n";
    guidance.examples.forEach((example: string, index: number) => {
...
function formatProjectAnalysis(analysis: any): string {
...
function formatBestPractices(practices: any, includeExamples: boolean): string {
  let output = `📋 **${practices.category.toUpperCase()} Best Practices**\n\n`;

  output += "**Practices:**\n";
  practices.practices.forEach((practice: string, index: number) => {
    output += `${index + 1}. ${practice}\n`;
  });

  if (includeExamples && practices.examples && practices.examples.length > 0) {
    output += "\n**Examples:**\n";
    practices.examples.forEach((example: string, index: number) => {
      output += `**Example ${index + 1}:**\n\`\`\`gdscript\n${example}\n\`\`\`\n\n`;
    });
  }
...
function formatPerformanceGuidance(guidance: any): string {
...
function formatWorkflowAssistance(assistance: any): string {
  ...
  if (assistance.nextSteps && assistance.nextSteps.length > 0) {
    output += "**Next Steps:**\n";
    assistance.nextSteps.forEach((step: string, index: number) => {
      output += `${index + 1}. ${step}\n`;
    });
    output += "\n";
  }
  ...
  if (assistance.tips && assistance.tips.length > 0) {
    output += "**Tips:**\n";
    assistance.tips.forEach((tip: string, index: number) => {
      output += `• ${tip}\n`;
    });
  }
